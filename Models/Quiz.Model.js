...
module.exports.findAll = (callBack) => {
  pool.query("SELECT * FROM `quiz` order by ID DESC", (err, result, fields) => {
    if (err) {
      return callBack(err);
    } else {
...
module.exports.insertOne = (quiz, callback) => {
  pool.query(
...
module.exports.DeleteOne = (quiz, callBack) => {
  pool.query("delete from quiz where ID =?", [quiz.ID], (err, result) => {
    if (err) {
      return callBack(err);
    } else {
      return callBack(null, result);
